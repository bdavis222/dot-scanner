...
class DialogWindow:
    def __init__(self, title, message, positiveButtonText, negativeButtonText, positiveButtonAction,
        ...
        self.window.geometry(
            f"{windowWidth}x{windowHeight}+{windowX}+{windowY}")
        ...
        if positiveButtonAction is None:
            ...
            self.positiveButton = tk.Button(
                self.window, text=positiveButtonText, fg=positiveButtonColor,
                command=positiveButtonAction)

        if negativeButtonAction is None:
            self.negativeButton = tk.Button(
                self.window, text=negativeButtonText, command=self.window.destroy)
            self.window.bind("<Escape>", self.closeWindowWithEscapeKey)
        else:
            self.negativeButton = tk.Button(
                self.window, text=negativeButtonText, command=negativeButtonAction)
